...
export const PROJECTS: ProjectItem[] = [
  {
    ...
    deepDescription:
      "Pick a topic you want to learn, and Spoin gives you a scrollable feed of bite-sized cards at your difficulty level, with an inline AI chat and personal notes to dig deeper on anything that catches your interest. 10,247 cards are live so far. The hard part is generation: an async pipeline keeps expensive LLM calls off the read path entirely, so what you actually swipe through comes straight from Postgres, pre-computed and deduplicated, in under 50ms. Generation itself runs behind a 2D quota grid across 16 verified free-tier API keys, with a fallback ladder per model and per-cell serialization (ADR-0040) so a burst of calls does not trip rate limits.",
    caseStudyHref: '/work/spoin',
    metrics: [
      { label: 'Corpus Size', value: '10,247 Cards', detail: '8,382 Questions in Postgres' },
