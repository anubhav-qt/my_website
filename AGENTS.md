...
> **Current status**: v3. All four pages (`/`, `/projects`, `/scratchpad`, `/contact`) are
...
has to hold from 280px up. The zoom brings two gotchas: `100vh` stays the unzoomed viewport
(so no `min-h-screen`), and `getBoundingClientRect` reports zoomed pixels. `index.css`
explains both where the rule lives.
